...
function ProductContainer({ children, ...props }, ref) {
   const { product, className, showSellNumber } = props;
   const { name, image, price, sale, sellNumber } = product;
   return (
      <div className={clsx('productContainer', className)} ref={ref}>
         ...
         <div className="productContainer__productInfo">
            ...
            <div
               className={clsx('productContainer__productInfo__price', { sale: sale && sale > 0 })}
            >
            ...
            {showSellNumber && typeof sellNumber === 'number' && (
               <div className="productContainer__productInfo__sellNumber">
                  {`Đã bán ${sellNumber} sản phẩm`}
               </div>
            )}
         </div>
         {sale && sale > 0 ? (
